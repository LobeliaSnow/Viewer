...
namespace Lobelia {
	...
	namespace Future {
		...
		private:
			void MeshSave(std::weak_ptr<Utility::FileController> file, ModelMolding::Mesh* mesh);
			void ClusterSave(std::weak_ptr<Utility::FileController> file, ModelMolding::Mesh* mesh);
		public:
			DxdExporter();
			~DxdExporter();
			void Save(ModelMolding* model, const char* file_path);
		};
